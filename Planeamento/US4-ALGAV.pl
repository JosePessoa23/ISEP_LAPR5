...
armazens_por_massa(Percurso):-get_armazens(Armazens),get_massas(Massas),sort_by_massas(Armazens,Massas,OrdemArmazens,_),adicionar_armazem_inicial_final(OrdemArmazens,Percurso),!.
...
% Coloca todas as massas das entregas na lista Massas.
get_massas(Massas):-findall(Massa,entrega(_,_,Massa,_,_,_),Massas).
...
armazens_por_tempo(Percurso):-get_armazens(Armazens),armazens_por_tempo(Armazens, 5, [], P), adicionar_armazem_inicial_final(P, Percurso).
...
calcula_tempo_massa(Percurso):- get_time(Ti), armazens_por_massa(Percurso), get_time(Tf), T is Tf-Ti, write('Tempo geracao da solucao:'), write(T).

calcula_tempo_tempo(Percurso):- get_time(Ti), armazens_por_tempo(Percurso), get_time(Tf), T is Tf-Ti, write('Tempo geracao da solucao:'), write(T).

calcula_tempo_tempo_e_massa(Percurso):- get_time(Ti), armazens_por_tempo_e_massa(Percurso), get_time(Tf), T is Tf-Ti, write('Tempo geracao da solucao:'), write(T).





rota_tempo_min_massa(Custo,LAF):-(run_massa();true),tempo_min(LAF,Custo).

run_massa():- retractall(tempo_min(_,_)), assertz(tempo_min(_,100000)),tempo_percurso_massa().

tempo_percurso_massa():-massa_entrega(MassaT),carateristicasCam(_,_,_,Energia,_,_),armazens_por_massa(Armazens),tempo_total(Armazens,Custo,MassaT,Energia),atualiza(Armazens,Custo),fail.



rota_tempo_min_tempo(Custo,LAF):-(run_tempo();true),tempo_min(LAF,Custo).

run_tempo():- retractall(tempo_min(_,_)), assertz(tempo_min(_,100000)),tempo_percurso_tempo().

tempo_percurso_tempo():-massa_entrega(MassaT),carateristicasCam(_,_,_,Energia,_,_),armazens_por_tempo(Armazens),tempo_total(Armazens,Custo,MassaT,Energia),atualiza(Armazens,Custo),fail.



rota_tempo_min_tempo_e_massa(Custo,LAF):-(run_tempo_e_massa();true),tempo_min(LAF,Custo).

run_tempo_e_massa():- retractall(tempo_min(_,_)), assertz(tempo_min(_,100000)),tempo_percurso_tempo_e_massa().

tempo_percurso_tempo_e_massa():-massa_entrega(MassaT),carateristicasCam(_,_,_,Energia,_,_),armazens_por_tempo_e_massa(Armazens),tempo_total(Armazens,Custo,MassaT,Energia),atualiza(Armazens,Custo),fail.
